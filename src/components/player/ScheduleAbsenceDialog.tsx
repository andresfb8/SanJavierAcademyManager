...
export function ScheduleAbsenceDialog({
      ...
      <Dialog open={open} onOpenChange={handleClose}>
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-4 py-2">
            {selected.getsRecovery ? (
              <div className="flex items-start gap-2 rounded-lg bg-emerald-50 border border-emerald-200 p-3 text-sm text-emerald-700">
                ...
                <span>Se generará <strong>+1 crédito de recuperación</strong> al avisar con más de 24h.</span>
              </div>
            ) : (
              <div className="flex items-start gap-2 rounded-lg bg-amber-50 border border-amber-200 p-3 text-sm text-amber-700">
                <Info className="h-4 w-4 mt-0.5 shrink-0" />
                <span>Menos de 24h de antelación — no se generará crédito de recuperación.</span>
              </div>
            )}
          ...
          <DialogFooter className="gap-2">
            <Button variant="outline" onClick={() => setSelected(null)}>Atrás</Button>
            <Button onClick={handleConfirm}>Confirmar ausencia</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    )
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-w-sm">
        <DialogHeader>
          <DialogTitle>Mis ausencias programadas</DialogTitle>
          <DialogDescription>
            Toca una clase para avisar que no podrás asistir. Toca de nuevo para cancelar el aviso.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-2 max-h-[50vh] overflow-y-auto py-1 pr-1">
          {upcomingClasses.length === 0 ? (
            ...
            upcomingClasses.map((uc) => {
              const isAbsent = existingSet.has(`${uc.group.id}__${uc.dateStr}`)
              return (
                <button
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className={cn(
                      ...
                      isAbsent ? 'bg-red-100 text-red-500' : 'bg-slate-100 text-slate-400',
                    )}>
                      <CalendarDays className="h-4 w-4" />
                    </div>
                    <div>
                      <p className={cn('text-sm font-bold leading-tight', isAbsent ? 'text-red-700' : 'text-slate-800')}>
                        {uc.group.name}
                      </p>
                  ...
                  {isAbsent ? (
                    <Badge variant="destructive" className="text-[10px] shrink-0">Avisado</Badge>
                  ) : uc.getsRecovery ? (
                    <Badge variant="outline" className="text-[10px] text-emerald-600 border-emerald-200 shrink-0">+recuperación</Badge>
                  ) : null}
                </button>
              )
            })
